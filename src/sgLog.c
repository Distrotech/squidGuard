...
#else
void sgLog(log, format, va_alist) struct LogFileStat *log;
char *format;
va_dcl
#endif
{
...
#else
void sgLogError(format, va_alist) char *format;
va_dcl
#endif
{
    char msg[MAX_BUF];
...
#else
void sgLogFatalError(format, va_alist) char *format;
va_dcl
#endif
{
    char msg[MAX_BUF];
...
void sgLogRequest(struct LogFile *log,
                  ...
                  int request)
#else
void sgLogRequest(log, req, acl, aclpass, rewrite, request) struct LogFile *log;
struct SquidInfo *req;
struct Acl *acl;
struct AclDest *aclpass;
struct sgRewrite *rewrite;
