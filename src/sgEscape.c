...
#else
void sgFreeDestination(dest) struct Destination *dest;
#endif
{
       if (dest->name != NULL)
              sgFree(dest->name);
       if (dest->domainlist != NULL)
              sgFree(dest->domainlist);
       if (dest->domainlistDb != NULL)
              sgFree(dest->domainlistDb);
       if (dest->urllist != NULL)
              sgFree(dest->urllist);
       if (dest->urllistDb != NULL)
              sgFree(dest->urllistDb);
       if (dest->expressionlist != NULL)
...
#else
void sgFreeSetting(set) struct Setting *set;
#endif
{
       if (set->name != NULL)
              free(set->name);
...
#else
void sgFreeTime(t) struct Time *t;
#endif
{
       if (t->name != NULL)
              free(t->name);
...
#else
void sgFreeRewrite(rew) struct sgRewrite *rew;
#endif
{
       if (rew->name != NULL)
              sgFree(rew->name);
...
#else
void sgFreeAcl(acl) struct Acl *acl;
#endif
{
       if (acl->name != NULL)
              sgFree(acl->name);
...
#else
void sgFreeAclDest(ad) struct AclDest *ad;
#endif
{
       if (ad->name != NULL)
              sgFree(ad->name);
...
#else
void sgFreeLogFileStat(lfs) struct LogFileStat *lfs;
#endif
{
       if (lfs->name != NULL)
              sgFree(lfs->name);
