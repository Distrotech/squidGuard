...
#else
void sgHandlerSigHUP(signal) int signal;
#endif
{
    sig_hup = 1;
}
...
#else
void sgAlarm(signal) int signal;
#endif
{
    sig_alrm = 1;
    sgTimeNextEvent();
...
#else
void *sgRealloc(ptr, elsize) void *ptr;
size_t elsize;
#endif
{
    void *p;
...
#else
void sgFreePatternBuffer(regexp) struct sgRegExp *regexp;
#endif
{
                ...
                p = strncat(newstring, pattern, pm[0].rm_so);
                {
                         ...
                         p_next++)
                    {
                        if (*p_next == '\\')
                        {
                            ...
                            p_next++;
                            if (p_next < (re->substitute + substlen) && '0' <= *p_next && *p_next <= '9')
                            {
                                int i = *p_next - '0';
...
#endif
{
    ...
    m--;
    y = y - 1900;
    if (y < 0 || m < 0 || m > 11 || d < 1 || d > 31 || H < 0 || H > 23 || M < 0 || M > 59 || S < 0 || S > 59)
        return (time_t)-1;
    t->tm_year = y;
    t->tm_mon = m;
    t->tm_mday = d;
