...
#else
void sgDbInit(Db, file) struct sgDb *Db;
char *file;
#endif
{
  struct stat st;
...
#else
void updateProgressBar(prog) float prog;
#endif
{
  if (1 == stdoutisatty)
  {
...
#else
void sgDbLoadTextFile(Db, filename, update) struct sgDb *Db;
char *filename;
int update;
#endif
{
...
#else
void sgDbUpdate(Db, key, value, len) struct sgDb *Db;
char *key;
char *value;
int len;
#endif
